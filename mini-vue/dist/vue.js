    ...
    function updateChildren(el, oldChildren, newChildren) {
      ...
      while (oldStartIndex <= oldEndIndex && newStartIndex <= newEndIndex) {
        if (!oldStartNode) {
          ...
          oldStartNode = oldChildren[++oldStartIndex];
          newStartNode = newChildren[++newStartIndex]; //比较开头节点
        } else if (isSameVnode(oldEndNode, newEndNode)) {
          patchVNode(oldEndNode, newEndNode);
          oldEndNode = oldChildren[--oldEndIndex];
          newEndNode = newChildren[--newEndIndex]; //比较尾部节点
        } else if (isSameVnode(oldEndNode, newStartNode)) {
          //交叉比对
          patchVNode(oldEndNode, newStartNode); //insertBefore具有移动性，会将原来的元素移动走
          ...
          var moveIndex = map[newStartNode.key]; //如果拿到则说明是我要移动的索引

          if (moveIndex !== undefined) {
            var moveNode = oldChildren[moveIndex]; //找到虚拟节点，复用
